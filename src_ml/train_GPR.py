...
# For the best performance, I scale the input parameters to be between 0 and 1 and the output (triple product) to have zero mean and unit variance.
# But this is has become a mess. Be very careful about the scaling and descaling of X and Y.

# One way to speed up the code is to store results of kernel computations, as they are repeatedly used in both hyperparameter optimisation and prediction.

# As there is no wrapper for METIS simulations here, the user has to run the simulations externally and place the results in the appropriate folders.
...
if __name__ == "__main__":
    ...
    x_best, mu_best_scaled, sigma_best_scaled = extract_maximum(
        scale_X(X_train), scale_Y(Y_train, sigma_n)[0], lengthscales, sigma_f, sigma_n, n_candidates=10000
        )
...
# Old code below, kept for reference only
    ...
    # ---------------------------------------
    data = read_simulation_folder('./data_ml/data_batch_ini')
    X_train = scale_X(data['X'])
    Y_train = data['triple']
    sigma_n = data['triple_err']
    # Scale X_train and Y_train, preserve mean and std for descaling later
    X_train = scale_X(X_train)
    Y_train, sigma_n, Y_mean_ini, Y_std_ini = scale_Y(Y_train, sigma_n)
    ...
    for round_idx in range(1, n_rounds + 1):
        ...
        # -----------------------------------
        # 2. Optimise GP hyperparameters
        # -----------------------------------

        # Note that X_train, Y_train, and sigma_n are already scaled
        lengthscales, sigma_f = minimise_neg_lml(
            X_train, Y_train, sigma_n, params0=np.concatenate([lengthscales, [sigma_f]])
        )
        ...
        # 3. Generate candidate points
        # -----------------------------------
        X_cand = generate_candidates_lhs(candidate_pool_size)
        X_cand_scaled = scale_X(X_cand)

        # -----------------------------------
        # 4. Predict GP mean & variance
        # I think the output mu and sigma are scaled
        # -----------------------------------
        mu, sigma = gp_predict(
            X_train, Y_train, X_cand_scaled,
            lengthscales, sigma_f, sigma_n
        )

        # ---------------------------------------
        # 5. Acquisition function → choose next batch
        # ---------------------------------------
        X_next_scaled = select_next_points(
            X_cand_scaled, mu, sigma,
            n_points=batch_size,
        ...
        print("Next points to sample (physical units):")
        print(X_next)

        # ---------------------------------------
        # 6. SAVE THESE POINTS FOR THE USER
        #    (so the simulation script can read them)
        # ---------------------------------------
        np.savetxt(f"./data_ml/batch_to_run/batch_{round_idx}.txt", X_next)

        print(f"→ Saved next batch to: ./data_ml/batch_to_run/batch_{round_idx}.txt")

        # -------------------------------------------------------
        # 7. Wait for simulations to finish externally
        #   create a folder ./data_ml/data_batch_{round_idx} for you
        #    Then load new results from: ./data_ml/data_batch_{round_idx}
        # -------------------------------------------------------
        new_folder = f"./data_ml/data_batch_{round_idx}"
        print(f"Please run the simulation for this batch and place results in {new_folder}.")
        input("Press Enter to continue once the simulation is finished and data is ready...")
        ...
        # ---------------------------------------
        # 8. Append new training data
        # ---------------------------------------
        X_new = scale_X(new_data['X'])
        Y_new = new_data['triple']
        sigma_new = new_data['triple_err']

        X_train = np.vstack([X_train, X_new])
        Y_train = np.concatenate([Y_train, Y_new])
        sigma_n = np.concatenate([sigma_n, sigma_new])

        print("Training dataset updated.")
        print("Current dataset size =", len(Y_train))

    # 9. Extract the maximum predicted triple product
    # -----------------------------
    x_best, mu_best_scaled, sigma_best_scaled = extract_maximum(
        X_train, Y_train, lengthscales, sigma_f, sigma_n, n_candidates=10000
        )

    # Descale predicted mean and uncertainty
    mu_best = descale_Y(mu_best_scaled, Y_mean_ini, Y_std_ini)
    sigma_best = descale_Y(sigma_best_scaled, Y_mean_ini, Y_std_ini)
